...
import { GetRoom, GetProduct, GetDamage, GetStatus, Checkrooms, UpdateCheckroom, GetCheckroomslist, GetChecklists } from "./service/service";
...
function CheckroomEdit() {
  const [checkroomID, setCheckroomID] = useState<CheckroomInterface[]>([]);
  ...
  const onChangeCR = async (event: SelectChangeEvent) => {
    const id = event.target.value
    let res = await GetCheckroomslist(id);
    if (res) {
      setCheckroom(res);
    }
  };
            ...
            <Grid container spacing={1} sx={{ marginBottom: 2 }}>
              ...
              >
                <FormLabel
                  ...
                  sx={{ marginRight: 4, fontSize: 17, paddingBottom: 1 }}
                >
                  Checklist:
                </FormLabel>
                <Select
                ...
                >
                  ...
                  {checkroomID.map((item: CheckroomInterface) => (
